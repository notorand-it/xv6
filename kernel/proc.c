...
proc_mapstacks(pagetable_t kpgtbl)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    // only map PGSIZE even though KSTACK added room for an extra page. That page will not be mapped so it can serve as a guard page
...
found:
  ...
  //Allocate a page for alarm trap frame page
...
/*
Set the track mask for the process
*/
void trace(int mask) {
  struct proc *p = myproc();
  p->tracemask = mask;
...
// Returns the number of processes whose state are not UNUSED
int nproc() {
  int count = 0;
  struct proc *p;
